 ...
 * `clan.ts` builds `TABLES` — one `Record<Outcome, string>` table per
 * action, `REFUSAL` spread in.
...
export const LEADERSHIP_TABLES = { "claim-succession": CLAIM, "open-vote": OPEN_VOTE, "cast-vote": CAST } as const;
export type LeadershipAction = keyof typeof LEADERSHIP_TABLES;
