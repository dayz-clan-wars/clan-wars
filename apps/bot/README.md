...
| `WEEKLY_VEHICLE_WIPE` | no (default off; `"1"`/`"true"` = on) | Wipe one of five vehicles each Monday, rotating weekly: Olga → Gunter → Hummer → Ada → Sarka. Uses the same window as `TRUCK_WIPE_OFF_HOUR`/`ON_HOUR`. Requires `RESTART_SCHEDULE` and `SERVER_EVENTS_CHANNEL_ID`; config load fails without either — the Sunday notice is the only warning a player gets before their vehicle is cleared. Independent of `TRUCK_WIPE_EVENTS`. Runbook `docs/deploy/2026-09-12-weekly-vehicle-rotation.md`. |
| `RAID_WINDOW_TICK` | no (default off; `"1"`/`"true"` = on) | Flip the raid window (base damage) on the restart slots and post its advance/open/close/failure notices to `SERVER_EVENTS_CHANNEL_ID`. Requires `RESTART_SCHEDULE` and `SERVER_EVENTS_CHANNEL_ID`; config load fails without either, because the flip only ever takes effect at a restart slot and the advance/open/close notices are the player-facing point of the feature. |
| `OPS_CHANNEL_ID` | no (default unset = error log only) | Channel the raid-window tick posts its failure alert to. Unset means the alert falls back to an error-level log line and nothing else, the same way `WAR_LOG_CHANNEL_ID` degrades when unset. |
| `ANNOUNCEMENTS_CHANNEL_ID` | not read by the bot | ⚠️ Kept in `/opt/clan-wars/.env` for the **manual** `#announcements` posting workflow a human runs from the production host, outside the bot entirely (see `docs/deploy/2026-09-21-server-events-channel.md`). The bot's own `cfg.announcementsChannelId` was removed on 2026-09-21 when the raid window and the weekly vehicle wipe moved to `SERVER_EVENTS_CHANNEL_ID` — do not delete this variable from the deployed environment on the strength of that; only the bot's read of it is gone. |
...
| `UNLINKED_PC_BAN` | no (default off; `"1"`/`"true"` = on) | Gates `pcBanTick` (every 5 minutes, beside the reaper and before `banTick`): bans a PC player with no linked Discord account, and lifts that ban once when they start linking. Requires `ENFORCEMENT_TICK`; config load fails if this is on without it, because a written `bans` row is never applied without `banTick` running. `BAN_DRY_RUN` still governs whether the resulting ban reaches Nitrado — there is no separate dry-run for this gate. Runbook `docs/deploy/2026-09-18-unlinked-pc-ban.md`. |
...
| `BANS_CHANNEL_ID` | no (unset means public ban announcements are off) | The public `#bans` channel that enforced bans and unbans are announced to (`ban_announcements`, drained by `banAnnounceTick` right after `banTick` every 5 minutes). `ban_announcements` rows are queued by real ban transitions regardless of this setting — a dry-run ban never queues one, but `ENFORCEMENT_TICK` alone is enough to start filling the queue. Unset by default: rows still accumulate, nothing posts — and ⚠️ they all post at once, oldest first, when a channel is first configured. The bot needs **View Channel and Send Messages** in that channel. Runbook `docs/deploy/2026-09-18-public-ban-announcements.md`. |
| `AIRDROP_TICK` | no (default off; `"1"`/`"true"` = on) | Decide and announce airdrop events on the restart slots. Requires both `RESTART_SCHEDULE` and `SERVER_EVENTS_CHANNEL_ID`; config load fails without either, because the spawner only ever takes effect at a restart and the location announcement is the only way a drop is ever found. |
...
| `AIRDROP_MIN_POP` | no (default `5`) | The floor under the trailing p90 population an airdrop requires before it is decided. Plain decimal digits only. |
...
This registers the bot's 18 slash commands — `/link`, `/base`, `/me`,
`/roster`, `/clan`, `/clans`, `/lead`, `/found`, `/guest`, `/vault`, `/map`,
`/scoreboard`, `/alphas`, `/seasons`, `/warlog`, `/player`, `/board` and
`/achievements` — each backed by a real handler over `@factions/roster`
(`apps/bot/src/commands/`, registry in `commands/index.ts`), logs in,
