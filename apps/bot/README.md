...
| `RAID_WINDOW_TICK` | no (default off; `"1"`/`"true"` = on) | Flip the raid window (base damage) on the restart slots and post its advance/open/close/failure notices. Requires `RESTART_SCHEDULE`; config load fails without it, because the flip only ever takes effect at a restart slot. |
...
| `SERVER_EVENTS_CHANNEL_ID` | only when `AIRDROP_TICK` is on | Channel the airdrop tick posts a drop's location to. Fatal, not degraded, when `AIRDROP_TICK` is on and this is unset — with no in-world marker, the announcement is the only way anyone ever finds the drop, so config load refuses to start. |
