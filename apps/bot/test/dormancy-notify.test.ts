...
describe("formatDormancyDm", () => {
  it("tells a dormant leader what happened, what to do, and the deadline", () => {
    const msg = formatDormancyDm(base, SITE);
  ...
  it("⚠️ interpolates the window rather than hardcoding 'seven days'", () => {
    ...
    expect(formatDormancyDm(base, SITE)).toMatch(/\b7 days\b/);
    expect(formatDormancyDm({ ...base, dormantAfterMs: 3 * 86_400_000 }, SITE)).toMatch(/\b3 days\b/);
    expect(formatDormancyDm({ ...base, dormantAfterMs: 86_400_000 }, SITE)).toMatch(/\b1 day\b/);
  });
  ...
  it("⚠️ reports a sub-day window in hours rather than rounding down to 0 days", () => {
    ...
    expect(formatDormancyDm({ ...base, dormantAfterMs: 6 * 3_600_000 }, SITE)).toMatch(/\b6 hours\b/);
    expect(formatDormancyDm({ ...base, dormantAfterMs: 6 * 3_600_000 }, SITE)).not.toMatch(/\bdays?\b/);
    expect(formatDormancyDm({ ...base, dormantAfterMs: 3_600_000 }, SITE)).toMatch(/\b1 hour\b/);
  });

  it("confirms a revival", () => {
    const msg = formatDormancyDm(revived, SITE);
    expect(msg).toMatch(/Bears/);
    expect(msg).toMatch(/supplies/i);
    expect(msg).not.toMatch(/returns to the pool/i);
  });

  it("links the clan to its page", () => {
    expect(formatDormancyDm(base, SITE)).toContain(`[Bears](<${SITE}/clans/BEAR>)`);
  });
  ...
  it("⚠️ never includes pole coordinates", () => {
    ...
    for (const notice of [base, revived]) {
      expect(formatDormancyDm(notice, SITE)).not.toMatch(/\d+\.\d+:\d+\.\d+:\d+\.\d+/);
    }
  });
});
...
describe("notifyDormancy", () => {
  it("⚠️ sends with an empty channel id, so there is no public fallback", async () => {
    ...
    await notifyDormancy([base], send, SITE);
  ...
  it("⚠️ a failed DM is reported and never retried", async () => {
    ...
    const onError = vi.fn();
    expect(await notifyDormancy([base], send, SITE, onError)).toBe(0);
  ...
  it("one unreachable leader does not stop the others being told", async () => {
    ...
    expect(await notifyDormancy([base, { ...base, factionId: 2, leaderDiscordId: "d2" }], send, SITE, vi.fn())).toBe(1);
