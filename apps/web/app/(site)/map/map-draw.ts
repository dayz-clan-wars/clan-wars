    ...
    const text = (age: string) =>
      `<div class="min-w-[13rem] max-w-[16rem]">` +
        ...
        `<form method="post" action="/api/map/pin/delete" class="m-0">` +
          ...
          `<button type="submit" class="flex min-h-[44px] w-full items-center justify-center border-t border-rule-2 font-display text-xs uppercase tracking-[0.06em] text-ink shadow-[inset_0_2px_0_var(--color-rust)] hover:bg-rust/15">Delete pin</button>` +
        `</form>` +
      `</div>`;
