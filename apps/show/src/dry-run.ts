...
/** The read-only review path (spec §10): `--dry-run` and/or `--render <dir>`. Returns the exit code. */
export async function runDryRun(opts: { week?: string; printPrompt: boolean; render?: string }): Promise<number> {
  const renderDir = opts.render;
  let exitCode = 0;
  ...
  const runImpl: Run | undefined = renderCfg
    ? (cmd, args, opts2) => spawnRun(cmd === "ffmpeg" ? renderCfg.ffmpegPath : cmd, args, opts2)
    : undefined;
  ...
  try {
    ...
    const weekStart = opts.week ? parseWeekArg(opts.week) : lastEndedWeek(new Date());
