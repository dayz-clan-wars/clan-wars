...
| The leaderboards in Discord (nine standing messages, edited in place) | `apps/bot/src/leaderboard-tick.ts` (reconciler + `PgLeaderboardStore`), `apps/bot/src/leaderboard-embed.ts` (pure render, key, and the URL→board lookup), the channel adapter `createLeaderboardChannel` in `discord.ts`, run beside the crowns on `LEADERBOARD_TICK_INTERVAL_MS` (default 5 min). Gated on `LEADERBOARDS_CHANNEL_ID`. ⚠️ **No stored message id** — the nine are rediscovered from the channel by the board link in each embed (same discovery approach as `createOnlineBoard`), so a restart adopts them instead of reposting. ⚠️ If ANY of the nine is missing the channel is purged and rebuilt, because order is only ever established at post time and a lone re-send lands at the bottom. ⚠️ A failed board read writes nothing; a board's key is written only after its edit succeeds. `BOARD_TOP`, `BOARD_SLUGS`, `boardKindFromSlug` and `boardValue` moved to `@factions/copy` for this (web re-exports them) — ⚠️ `packages/copy` must import no RUNTIME value from `@factions/roster`, see `packages/copy/test/leaf.test.ts`. Runbook `docs/deploy/2026-09-18-leaderboards-in-discord.md` |
