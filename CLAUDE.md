...
- **Web app:** `docker compose build web && docker compose up -d web`. It publishes
  ...
  /player /board /achievements`, plus the admin-only (ManageGuild) `/airdrop`, `/award`, `/bounty` and `/koth`. `apps/bot/test/parity.test.ts` lists every write and
...
- **Lock order (spec §4.12): `factions` → `declarations` → `poles` →
  ...
  `achievement_counters` → `koth_events` → `award_grants` → `faction_events` → `war_log_events` → `bounties` → `clan_notices` →
