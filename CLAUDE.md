...
Expect **32/32 tasks** (`packages/roster`, `packages/declarations`, `packages/copy`,
`packages/deploy` and `apps/show` each add `typecheck` and `test`). A cached pass
...
**CI runs that same gate on every PR** (`.github/workflows/ci.yml`, since 2026-09-16),
on node 20 against a throwaway Postgres 16 service container. Two things about it are
worth knowing before you read a red check:

- ⚠️ **The `factions_test_<package>` collision hazard above does not apply in CI, and
  must not be "fixed" there.** That warning is about two runs on *this machine* sharing a
  database name derived from the package alone. Every CI run gets its own service
...
expands the job name, and a rule requiring the unexpanded form waits forever on a check
that never reports. The branch flow is keel's: `keel:start-work` to branch,
`keel:finish-work` to open the PR, `keel:review`, then `keel:land`. Under this repo's
trunk topology `keel:land` merges with `--merge`, never `--squash` — `--squash` is blocked.

⚠️ **Every PR must add an entry under `## [Unreleased]` in `CHANGELOG.md`, committed —
not merely edited.** The gate diffs against the committed state at the PR base, so a
staged or on-disk edit does not satisfy it.

⚠️ **`enforce_admins` is false, so an admin can bypass all of the above.** That is
deliberate — `reviewPolicy` is `review`, and GitHub forbids self-approval, so a solo
maintainer merging their own commented-on PR needs that door open. It does mean branch
protection here is a guardrail against mistakes, not a lock.

⚠️ **Secret scanning only ever sees the triggering event's commit range.** A secret
committed before 2026-09-16 is not found by `security.yml`, and its green check says
nothing about the history. The whole history was swept once at adoption (2026-09-16,
7,248 chunks): **0 verified, 7 unverified, all benign** — the `factions:factions` compose
credential, and a `config.ts` comment that spells out a credentials-in-URL example
while explaining why such URLs are rejected. (That comment is also why this file must
not spell one out literally: the scanner flags an unverified `URI` hit and, because
`security.yml` runs with `--results=verified,unknown`, an unverified hit **fails the
check**. Describe the shape; never write one out.) `.env` and `.env.production` have never been
committed. Sweep again by hand after any incident:
`docker run --rm -v "$PWD:/repo" -w /repo trufflesecurity/trufflehog:latest git
file:///repo --results=verified,unknown`. Rotate anything it finds — rewriting history
does not un-leak a pushed secret. trufflehog rather than gitleaks because this repo is
organization-owned, and the gitleaks action exits 1 without a licence key before scanning
anything.
...
- **Bot:** not containerised. `set -a && . ./.env && set +a && pnpm --filter @factions/bot start`.
  ...
  `SERVER_EVENTS_CHANNEL_ID` (the same channel airdrops and the weekly vehicle wipe
  post to, since `docs/deploy/2026-09-21-server-events-channel.md`) plus a
  once-per-boundary failure alert to `OPS_CHANNEL_ID` (or an error-level log line if
  that is unset). See `docs/deploy/raid-window.md`.
...
- **⚠️ Migrations are applied automatically on a release deploy, and by nothing
  ...
  **`pnpm db:migrate`** (`scripts/migrate.ts`) is the migration path either
  ...
  ⚠️ **A change to `deploy/deploy-release.sh` itself never takes effect on the deploy
  that introduces it.** The script re-execs from a private copy of itself at the very
  start, before it moves the tree to the new tag — the v1.16.6 fix, so that replacing
  the file mid-run (bash reads a script lazily by byte offset) can't resume the
  interpreter at a stale offset in new bytes. The consequence nobody wrote down until
  now: **the deployer that ships tag N is tag N−1's deployer.** Confirmed 2026-09-18:
  v1.18.0 added a `pnpm release:sync` call to this script's bookkeeping block, and it
  did not run, because v1.17.0's copy of the script executed the v1.18.0 deploy. This
  applies to ANY future change to `deploy-release.sh`, not just that one — a step added
  to it ships inert on its own release and starts working from the next one.
...
- **Web app:** `docker compose build web && docker compose up -d web`. It publishes
  ...
  /player /board /achievements /kothvote`, plus the admin-only (ManageGuild) `/airdrop`, `/award`, `/bounty` and `/koth`. `apps/bot/test/parity.test.ts` lists every write and
...
| Achievements (50, lifetime) | Definitions `packages/domain/src/achievements.ts` (incl. `ACHIEVEMENT_GROUP_COLORS`, the one statement of the four badge colours — the site's `--color-achievement-*` tokens and the bot's embed colour both read it); rules and tick `apps/bot/src/achievements/`; unlocks in `achievement_unlocks`; site via `achievementsFor()`. Badge art `apps/web/public/achievements/{unlocked,locked,svg}/<key>`, drawn inline by `apps/web/app/components/achievement-badge.tsx` from `lib/achievement-glyphs.ts` (generated by the web `prebuild` from the SVGs — rerun and commit after editing one); the Discord card `apps/bot/src/achievement-embed.ts`; the share card `apps/web/app/api/og/achievement/[key]/route.tsx`. Backfill `pnpm backfill:achievements`. Runbook `docs/deploy/2026-09-12-achievements.md` |
| Scheduled restarts (even UTC hours) | `apps/bot/src/restart-tick.ts`, gated on `RESTART_SCHEDULE`; slots from `restartSlot()` in `packages/domain/src/restarts.ts` (`RESTART_PERIOD_MS`, `RESTART_GRACE_MS` in `rules.ts`); one row per server per slot in `server_restarts`. `messages.xml` is never touched. The site counts down to the next slot in the timer bar (`apps/web/app/components/timer-bar.tsx`, from `nextRestartAt()` beside `restartSlot()`) — ⚠️ shown ONLY when `restartsScheduled()` (`packages/roster/src/restarts.ts`) finds a `server_restarts` row inside the last day, because the countdown is pure epoch arithmetic and would otherwise paint a confident clock on an install where `RESTART_SCHEDULE` is unset. Runbook `docs/deploy/2026-09-12-scheduled-restarts.md` |
...
| Public ban announcements (the `#bans` channel) | `ban_announcements` queue table (migration `0040_silent_slapstick.sql`), written inside `banTick`'s own transaction right after the `bans` row it is reporting on (`apps/bot/src/ban-tick.ts`); rendered by the pure `banAnnouncementText` (`apps/bot/src/ban-announce-text.ts`); drained by `banAnnounceTick` (`apps/bot/src/ban-announce-tick.ts`), run per server in `discord.ts` immediately after `banTick` in the same 5-minute block, oldest first, stopping at the first failure. Gated on `BANS_CHANNEL_ID` — the queue fills whether or not that is set; a dry-run ban never queues an announcement. The poster suppresses all mentions (`allowedMentions: { parse: [] }`) because a gamertag is player-controlled text landing in a public channel. Runbook `docs/deploy/2026-09-18-public-ban-announcements.md` |
| Clan role colours | `FLAG_COLORS` + `flagColor()` in `packages/domain/src/flags.ts` (33 entries, one per claimable flag), held by `packages/domain/test/flag-colors.test.ts` for coverage, uniqueness and WCAG 3.0 on BOTH Discord backgrounds. Applied by `structure-tick.ts`: passed to `createRole` at create, and repaired in step 3 beside the `mentionable` repair (`colorsFixed`). ⚠️ The colours are NOT derived from the flag art — the art is too dark and too clustered; the flag is only a stable unique key, so two clans can never share a colour. ⚠️ The table is a LITERAL, not an index into `CLAIMABLE_FLAGS`, so reordering that list cannot silently reassign every clan's colour. ⚠️ The repair skips when `roleColor` returns null (blind cache) — null is not 0. ⚠️ Discord paints a name with the highest COLOURED role, so colouring Admin (pos 20), Linked (17) or Alpha (16) again would hide every clan colour; all guild roles were set to default on 2026-09-18 to make this work. Runbook `docs/deploy/2026-09-18-clan-role-colors.md` |
| The leaderboards in Discord (ten standing messages, edited in place) | `apps/bot/src/leaderboard-tick.ts` (reconciler + `PgLeaderboardStore`), `apps/bot/src/leaderboard-embed.ts` (pure render, key, and the URL→board lookup), the channel adapter `createLeaderboardChannel` in `discord.ts`, run beside the crowns on `LEADERBOARD_TICK_INTERVAL_MS` (default 5 min). Gated on `LEADERBOARDS_CHANNEL_ID`. ⚠️ **No stored message id** — the ten are rediscovered from the channel by the board link in each embed (same discovery approach as `createOnlineBoard`), so a restart adopts them instead of reposting. ⚠️ If ANY of the ten is missing the channel is purged and rebuilt, because order is only ever established at post time and a lone re-send lands at the bottom. ⚠️ A failed board read writes nothing; a board's key is written only after its edit succeeds. `BOARD_TOP`, `BOARD_SLUGS`, `boardKindFromSlug` and `boardValue` moved to `@factions/copy` for this (web re-exports them) — ⚠️ `packages/copy` must import no RUNTIME value from `@factions/roster`, see `packages/copy/test/leaf.test.ts`. Runbook `docs/deploy/2026-09-18-leaderboards-in-discord.md` |
| The ten leaderboard crowns (a role per board, held by its #1) | The reconciler `apps/bot/src/crown-tick.ts` and its read `apps/bot/src/crown-store.ts`, run in `discord.ts` right after `structureTick` on its own `CROWN_TICK_INTERVAL_MS` throttle (default 5 min). Role ids come from ten optional `CROWN_*_ROLE_ID` env vars (including `CROWN_BOUNTY_KILLS_ROLE_ID`); a board with no id is never touched, so the last holder keeps that role forever. Scope is `{ kind: "current" }`, so a season rollover strips all ten at once. ⚠️ The store reads `playerBoardsDb` through `@factions/roster/internal` — the SAME query the public `/players` boards render, deliberately, so the crown and the board cannot drift. ⚠️ A failed board read returns early with NO writes; treating it as "nobody is #1" would strip every crown on one bad query. ⚠️ Ties are read `TIE_LIMIT` = 10 deep. Runbook `docs/deploy/2026-09-18-leaderboard-crowns.md` |
...
| No combat at the Hub (a hit, kill or trap there is a one-hour ban; Hub kills score nowhere) | Zone and offence rule `packages/domain/src/hub.ts` (numbers in `rules.ts`: a 100 m, ≥ 900 m-altitude cylinder — the Hub floats, the ground below is legal), positions on hit/kill payloads (`posInIdentity`, `packages/adm-parser`), the ban tick `apps/bot/src/hub-tick.ts` gated on `HUB_BAN_TICK` (refuses without `ENFORCEMENT_TICK`; writes `bans` rows with `reason = 'hub_combat'` for `banTick`; two minutes' return fire is self-defence), `kills.at_hub` (migration 0047) excluded beside `friendly_fire` in `scoringKill` — ⚠️ only from `HUB_COMBAT_FROM` (`rules.ts`) on, via `hubKillDiscredited`: kills before the rule were re-credited on 2026-09-24 (spec §2.6 amendment), and `rebuild:kills` recomputes the flag from that constant, so never set `at_hub` by hand, the three streak reads, the PvP achievements and the feeds. ⚠️ The ban hour runs from when the bot PROCESSES the offence, never the event's time — log delay must not eat it. ⚠️ `hub-watch` seeds itself at the log head on first run; `HUB_OFFENCE_MAX_AGE_MS` is only the backstop. ⚠️ Payload keys are `victimPos`/`attackerPos`/`killerPos`, never `pos` (`readFix` would take it for a map fix). One-time retro fix: `backfill-hub-positions.ts` → `rebuild:kills` → `revoke-achievements.ts`. Spec `docs/superpowers/specs/2026-09-22-hub-combat-design.md`, runbook `docs/deploy/2026-09-22-hub-combat.md` |
| Bounties (an admin punishment: announced, on everyone's map, collected by a kill) | Rule `packages/domain/src/bounties.ts` (`bountyOutcome` — claim before expiry, and a `BOUNTY_EXPIRY_SETTLE_MS` wait for log lag); store `packages/roster/src/internal/bounty-admin.ts`; `/bounty` in `apps/bot/src/commands/bounty.ts`; `bounty-tick.ts` (after `killsTick`) and `bounty-announce-tick.ts` (after the restart tick); the map layer in `packages/roster/src/map.ts`; the `bountyKills` board. Gated on `BOUNTY_TICK`, which requires `SERVER_EVENTS_CHANNEL_ID`. ⚠️ The map layer is a deliberate exception to §10.3 — a stranger's exact position, but only while their bounty is `open` AND they are ONLINE (an open `player_sessions` row on that server); both predicates are in the same WHERE clause in `mapStateDb`, never a filter afterwards, so a target who logs off leaves no pin on their base. ⚠️ A claim is `scoringKill`, imported, never respelled, and frozen when written: no cursor over `kills`, because `rebuild:kills` renumbers them. Spec `docs/superpowers/specs/2026-09-23-bounties-design.md`, runbook `docs/deploy/2026-09-23-bounties.md` |
| King of the Hill (an admin schedules a one-session arena event at a Livonia town: hill spawns, KotH loadouts, infected and predators converge, top killer wins the prize chosen at scheduling — any `awards.json` award, or none) | Rule `packages/domain/src/koth.ts`, numbers in `rules.ts`; the restart-tick convergence `apps/bot/src/koth-converge.ts` (`planKoth`/`convergeKothFiles`); scoring and the award transaction `apps/bot/src/koth-score.ts`; the tick `apps/bot/src/koth-tick.ts`; posts `apps/bot/src/koth-text.ts`; `/koth schedule\|cancel\|status` in `apps/bot/src/commands/koth.ts`; `koth_events` (migration `0049_gifted_sheva_callister.sql`; `0050_square_the_executioner.sql` makes `koth_events_slot_uq` partial, so a cancelled or failed row frees its slot; `0051_lumpy_zodiak.sql` adds `award_key` — the per-event prize, null for none — and the `finished` state, a scored session with nothing granted). ⚠️ With no prize the top killer wins whether or not they are linked; with one, only a linked player can. A prize that left the catalogue before scoring ends `finished` with an ops alert, never a throw — a refused grant would retry forever and the results would never post. Gated on `KOTH_TICK`, which requires `RESTART_SCHEDULE` and `SERVER_EVENTS_CHANNEL_ID`. ⚠️ **The bot owns the four whole files on the server for the session's duration** (`cfgplayerspawnpoints.xml`, `env/wolf_territories.xml`, `env/bear_territories.xml`, `env/zombie_territories.xml`) — every slot compares each against `koth/default/` and reverts a hand-edit within one slot, the same rule the raid window and airdrops established for their own files. ⚠️ `applyEvents` (the truck wipe's `events.xml` round trip, renamed) and `applyGameplay` are the ONLY round trips for their files — a second independent download/upload of either would silently discard whichever edit landed first. ⚠️ The restore arm runs **whether or not `KOTH_TICK` is set**, whenever any KotH row has ever existed on the server — switching the feature off mid-event must still put the server back. Only the OPENING is gated on it (`restartTick`'s `koth.open`, absent = off). ⚠️ A session **never opens late**: if the bot misses the opening slot, the row goes `failed` and a cancellation posts, unlike the raid window, which opens late rather than not at all. ⚠️ The `koth-` preset-file prefix is reserved — no other spawn-gear preset may ever start with it, because the restore arm uses it to recognise KotH's own entries in `spawnGearPresetFiles`. Spec `docs/superpowers/specs/2026-09-23-king-of-the-hill-design.md`, runbook `docs/deploy/2026-09-23-king-of-the-hill.md`. Since 2026-09-24 an event can also start **automatically** (`apps/bot/src/koth-decide-tick.ts`, gated on `KOTH_AUTO_TICK`: airdrop-style high-water, 24 h gap, `KOTH_WEEKLY_CAP` counting `origin = 'auto'` only) or by a **player vote** (`/kothvote`, `apps/bot/src/koth-vote-tick.ts`, gated on `KOTH_VOTE`: linked in-game players frozen at open, floor max(5, half), two-thirds Yes). Both are prize-less. ⚠️ Both ticks run BEFORE `airdropTick` — KotH takes a record night's slot, and the airdrop then may not fire for up to `AIRDROP_HISTORY_MS`. ⚠️ `koth_votes_slot_uq` is unconditional: one vote per slot, ever. Spec `docs/superpowers/specs/2026-09-24-koth-auto-and-vote-design.md`, runbook `docs/deploy/2026-09-24-koth-auto-and-vote.md`. |
| The weekly show (Boris and Pavel's animated recap) | `apps/show`: story context `src/story/`, screening `src/screening/`, prompt `src/prompt/`, script `src/script/`; the state machine `src/stages/` and publishing `src/engine/publish/` (YouTube, Discord forum, Facebook); tables `show_episodes`, `show_pronunciations`, `show_text_screening`. Deploy: `deploy/systemd/clan-wars-show.service` + `.timer` (oneshot, every 10 minutes), runbook `docs/deploy/2026-09-26-weekly-show.md`. ⚠️ Nothing from a `faction_events` payload may reach the context (spec §5.2). ⚠️ Ops-channel posts never carry raw blocked text — counts and categories only; `pnpm show:screening --show <date>` prints the raw reasons, in the terminal only. ⚠️ An episode awaiting approval blocks every later week (spec §8.1). ⚠️ The command is always `pnpm run show`, never `pnpm show` — the latter is pnpm's own `view` command. `pnpm run show --week <date> --dry-run` is read-only. Spec `docs/superpowers/specs/2026-09-25-weekly-show-design.md`, plans `docs/superpowers/plans/2026-09-25-weekly-show-*.md` |
...
- **Lock order (spec §4.12): `factions` → `declarations` → `poles` →
  ...
  `achievement_counters` → `koth_votes` → `koth_vote_voters` → `koth_events` → `award_grants` → `faction_events` → `war_log_events` → `bounties` → `clan_notices` →
  `zone_incidents` → `zone_incident_participants` → `bans`.**
  `koth_events` is written by the restart tick, `/koth` and `koth-tick`; the award transaction
  takes `koth_events` → `award_grants` → `clan_notices`.
  `koth_votes`/`koth_vote_voters` are written by `/kothvote`, the ballot handler and
  `koth-vote-tick`; the pass transaction takes `koth_votes` → `koth_vote_voters` →
  `koth_events`, and `removeFromGuildDb` deletes voter rows immediately before
  `award_grants`.
  `bounties` is written by `bounty-admin.ts` and `bounty-tick.ts`, each touching `bounties`
  then `clan_notices` only.
  ...
  `show_episodes`, `show_pronunciations` and `show_text_screening` are outside the order: written only by `apps/show`, one table per statement.
