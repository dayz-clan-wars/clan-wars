...
| Per-clan spawn armbands | `applyArmbands` in `apps/bot/src/restart-tick.ts`, riding the restart slots; the roster query is `apps/bot/src/armband-roster.ts` (full members only, `armbandFor()` from `@factions/domain`), the mission script is `apps/bot/src/init-c.ts`. Gated on `CLAN_ARMBANDS` (refuses to load without `RESTART_SCHEDULE`). ⚠️ The hook is **`OnClientNewEvent`**, not `StartingEquipSetup` or `EquipCharacter`: with `spawnGearPresetFiles` set in `cfggameplay.json`, vanilla `OnClientNewEvent` returns right after `ProcessEquipmentData` and never reaches either — a hook there equips nothing and reports success. ⚠️ Codegen is confined to validated string literals inside a lookup function; everything else in the file is hand-written and fixed, because a syntax error in `init.c` stops the mission loading on a file the bot rewrites unattended. ⚠️ The bot is now the SOLE writer of `init.c` — the `livonia` repo excludes it from its FTP deploy, so a Release no longer ships it. Runbook `docs/deploy/2026-09-12-clan-armbands.md` |
