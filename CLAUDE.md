...
| Per-clan spawn armbands | `applyArmbands` in `apps/bot/src/restart-tick.ts`, riding the restart slots; roster query `apps/bot/src/armband-roster.ts` (full members only, `armbandFor()`), mission script `apps/bot/src/init-c.ts`. Gated on `CLAN_ARMBANDS` (refuses to load without `RESTART_SCHEDULE`). ⚠️ **Three** hooks, all calling `CW_GiveArmband` after their `super`: `OnClientNewEvent` (character created on connect), `OnClientRespawnEvent` (**new character after death** — hooking only the first shipped a silent no-op on 2026-09-13) and `OnClientReadyEvent` (existing saved character loaded; the reachable equivalent of `PlayerBase.OnStoreLoad`, which would need a mod console DayZ does not allow). `StartingEquipSetup` and `EquipCharacter` are dead code while `cfggameplay.json` sets `spawnGearPresetFiles`. ⚠️ Every branch of the equip path `Print`s `[CLANWARS] …` — a silent no-op there is indistinguishable from the hook never running. ⚠️ Codegen is confined to validated string literals; the rest is fixed text, ASCII and CRLF, because a syntax error stops the mission loading on a file the bot rewrites unattended. ⚠️ The bot is the SOLE writer of `init.c` — the `livonia` repo excludes it from its FTP deploy. ⚠️ Nitrado's file server does not show a running session's `script_*.log` until it flushes at session end; an empty log directory is NOT evidence the mission failed to load. Runbook `docs/deploy/2026-09-12-clan-armbands.md` |
